...
class Solution(object):
    ...
    def solve(self):
        with open(self.filename) as f:
            ...
            non_won_board_idxs = set([num for num in range(len(bingo_boards))])

            for num in bingo_nums:
                board_idxs_to_process = non_won_board_idxs.copy()

                for board_idx in board_idxs_to_process:
                    board = bingo_boards[board_idx]
                    r, c = self._markBoard(board, num)

                    if r >= 0 and self._checkWon(board, r, c):
                        non_won_board_idxs.remove(board_idx)
                        if not non_won_board_idxs:
